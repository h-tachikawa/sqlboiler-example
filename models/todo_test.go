...
func testTodoToOneUserUsingUser(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, todoDBTypes, true, todoColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Todo struct: %s", err)
	}
...
func testTodoToOneRemoveOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	todoDBTypes = map[string]string{`CreatedAt`: `datetime`, `DueDate`: `timestamp`, `Finished`: `tinyint`, `ID`: `int`, `Note`: `text`, `Title`: `varchar`, `UpdatedAt`: `datetime`, `UserID`: `int`}
	_           = bytes.MinRead
)
